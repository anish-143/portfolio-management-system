...
def register():
    ...
    email = request.form.get('new_email')  # Using email as username
    ...
    conn.execute("SELECT * FROM user_profile WHERE username = %s", (username,))
    ...
    conn = mysql.connection.cursor()
    conn.execute("INSERT INTO user_profile (username, email, phone, user_password) VALUES (%s, %s, %s, %s)", (username, email, phone, hashed_password))
    mysql.connection.commit()
    conn.close()
